...
<main class="ml-64 min-h-screen flex flex-col">
    ...
    <%
        request.setAttribute("pageTitle", "Item Ledger");
    ...
    <section class="mt-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <div class="flex flex-wrap items-end justify-between gap-4">
            <div>
                <h2 class="text-2xl font-bold tracking-tight">Ledger view</h2>
                <p class="mt-1 text-sm text-slate-600">Inventory records with direct moderation actions.</p>
            </div>
        ...
        <div class="mt-6 grid gap-4 lg:grid-cols-2">
            ...
            <article class="rounded-3xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
                <div class="flex flex-wrap items-start justify-between gap-3">
                    <div>
                        <p class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Item <%= item.getItemId() %></p>
                        <h3 class="mt-1 text-xl font-bold text-slate-900"><%= escapeHtml(item.getTitle()) %></h3>
                        <p class="mt-1 text-sm text-slate-600"><%= escapeHtml(item.getType()) %> | <%= escapeHtml(item.getCategory()) %></p>
                    </div>
                ...
                <div class="mt-4 grid gap-3 sm:grid-cols-2">
                    <div class="rounded-2xl bg-white p-4">
                        <p class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Reporter</p>
                        <p class="mt-2 font-semibold text-slate-900"><%= escapeHtml(item.getReporterUsername()) %></p>
                    </div>
                    <div class="rounded-2xl bg-white p-4">
                        <p class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Location</p>
                        <p class="mt-2 font-semibold text-slate-900"><%= escapeHtml(item.getLocationName()) %></p>
                    </div>
                </div>

                <div class="mt-4 rounded-2xl bg-white p-4 text-sm text-slate-600">
                    <p class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Moderation status</p>
                    <p class="mt-2 font-semibold text-slate-900"><%= escapeHtml(item.getModerationStatus()) %></p>
                    <p class="mt-2 text-xs text-slate-500">Created: <%= escapeHtml(item.getCreatedAt()) %></p>
                </div>

                <form method="post" action="<%= contextPath %>/admin/items/moderate" class="mt-5 flex flex-wrap items-center gap-2">
