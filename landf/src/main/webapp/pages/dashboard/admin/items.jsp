...
<head>
    ...
    <title>Admin Items | Lost and Found</title>
    ...
    <header class="rounded-2xl border border-slate-200 bg-white px-6 py-5">
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
                ...
                <h1 class="mt-1 text-2xl font-bold">Item Moderation</h1>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-slate-200 text-sm">
                ...
                <tr class="text-left text-xs font-semibold uppercase tracking-[0.15em] text-slate-500">
                    ...
                    <th class="px-3 py-3">Moderation</th>
                </tr>
                </thead>
                ...
                <tr class="align-top">
                    <td class="px-3 py-3">
                        ...
                        <p class="mt-1 text-xs text-slate-500">ID: <%= item.getItemId() %> | Type: <%= escapeHtml(item.getType()) %></p>
                    ...
                    <td class="px-3 py-3 text-slate-700"><%= escapeHtml(item.getReporterUsername()) %></td>
                    <td class="px-3 py-3">
                        <p>
                            <span class="rounded-md border border-slate-300 bg-slate-50 px-2 py-1 text-xs font-semibold text-slate-700">
                                <%= escapeHtml(item.getStatus()) %>
                            </span>
                        </p>
                        <p class="mt-2">
                            <span class="rounded-md border border-slate-300 bg-slate-50 px-2 py-1 text-xs font-semibold text-slate-700">
                                <%= escapeHtml(item.getModerationStatus()) %>
                            </span>
                        </p>
                    </td>
                    <td class="px-3 py-3">
                        <form method="post" action="<%= request.getContextPath() %>/admin/items/moderate" class="flex items-center gap-2">
                            <input type="hidden" name="itemId" value="<%= item.getItemId() %>">
                            <select name="action" class="rounded-lg border border-slate-300 px-3 py-2 text-xs outline-none ring-sky-500 focus:ring-2">
                                <option value="close">Close</option>
                                <option value="reopen">Reopen</option>
                                <option value="flag">Flag</option>
                                <option value="mark_spam">Mark Spam (-50)</option>
                            </select>
                            <button type="submit"
                ...
                <% if (items.isEmpty()) { %>
                <tr>
                    <td colspan="4" class="px-3 py-8 text-center text-sm text-slate-500">No item records available.</td>
                </tr>
                <% } %>
                </tbody>
            </table>
