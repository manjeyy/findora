...
<main class="ml-64 min-h-screen flex flex-col">
    ...
    <%
        request.setAttribute("pageTitle", "Reports & Moderation");
    ...
    <section class="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <article class="rounded-2xl border border-slate-200 bg-white p-5">
            ...
            <p class="mt-3 text-4xl font-bold text-slate-900"><%= reports.size() %></p>
        </article>
        <article class="rounded-2xl border border-slate-200 bg-white p-5">
            <p class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Pending Review</p>
            <p class="mt-3 text-4xl font-bold text-slate-900"><%= pendingReports %></p>
        </article>
        <article class="rounded-2xl border border-slate-200 bg-white p-5">
            <p class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Resolved</p>
            <p class="mt-3 text-4xl font-bold text-slate-900"><%= reports.size() - pendingReports %></p>
        </article>
        <article class="rounded-2xl border border-slate-200 bg-white p-5">
            <p class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Focus</p>
            <p class="mt-3 text-4xl font-bold text-slate-900">Resolve</p>
        </article>
    </section>
    ...
    <section class="mt-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <div class="flex flex-wrap items-end justify-between gap-4">
            <div>
                <h2 class="text-2xl font-bold tracking-tight">Report moderation queue</h2>
                <p class="mt-1 text-sm text-slate-600">Inspect each report target and then resolve or reject it from the same row.</p>
            </div>
        </div>
        ...
        <div class="mt-6 overflow-x-auto rounded-2xl border border-slate-200">
            <table class="min-w-full divide-y divide-slate-200 text-sm">
                ...
                <tr class="align-top hover:bg-slate-50">
                    <td class="px-4 py-4">
                        <p class="font-semibold text-slate-900"><%= escapeHtml(report.getReason()) %></p>
                        <p class="mt-1 text-xs text-slate-500">Report ID: <%= report.getReportId() %></p>
                        <p class="text-xs text-slate-500">Reporter: <%= escapeHtml(report.getReporterUsername()) %></p>
                        <p class="text-xs text-slate-500">Created: <%= escapeHtml(report.getCreatedAt()) %></p>
                    </td>
                    <td class="px-4 py-4">
                        <p class="text-xs text-slate-600">Type: <span class="font-semibold text-slate-900"><%= escapeHtml(report.getTargetType()) %></span></p>
                        <p class="mt-1 text-xs text-slate-600">Target ID: <span class="font-semibold text-slate-900"><%= report.getTargetId() %></span></p>
                    </td>
                    <td class="px-4 py-4">
                        <span class="rounded-full border border-slate-300 bg-slate-50 px-3 py-1 text-xs font-semibold text-slate-700"><%= escapeHtml(report.getStatus()) %></span>
                        <% if (report.getReviewedBy() != null && !report.getReviewedBy().isBlank()) { %>
                        <p class="mt-2 text-xs text-slate-500">Reviewed by <%= escapeHtml(report.getReviewedBy()) %></p>
                        <% } %>
                        <% if (report.getReviewNotes() != null && !report.getReviewNotes().isBlank()) { %>
                        <p class="mt-1 text-xs text-slate-600"><%= escapeHtml(report.getReviewNotes()) %></p>
                        <% } %>
                    </td>
                    <td class="px-4 py-4">
                        <% if ("pending".equalsIgnoreCase(report.getStatus())) { %>
                        <form method="post" action="<%= contextPath %>/admin/reports/review" class="space-y-2">
                            ...
                            <div class="flex flex-wrap gap-2">
                                <button type="submit" name="decision" value="resolved" class="rounded-xl border border-emerald-600 bg-emerald-600 px-4 py-2 text-xs font-semibold text-white">Resolve</button>
                ...
                <% if (reports.isEmpty()) { %>
                <tr>
                    <td colspan="4" class="px-4 py-10 text-center text-sm text-slate-500">No report records available.</td>
                </tr>
                <% } %>
                </tbody>
            </table>
