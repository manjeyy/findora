...
<main class="mx-auto flex min-h-screen w-full max-w-md items-center px-6 py-12">
    <section class="w-full rounded-2xl border border-slate-200 bg-white p-8">
        <div class="mb-8"><h1 class="mt-2 text-3xl font-bold tracking-tight text-slate-900">Create account</h1>
            <p class="mt-2 text-sm text-slate-600">Set up your credentials for the Lost and Found system.</p></div>
        <% if (error != null && !error.isBlank()) { %>
        <div class="mb-4 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-medium text-rose-700"><%= escapeHtml(error) %>
        </div>
        <% } %>
        ...
        <p class="mt-6 text-center text-sm text-slate-600"> Already have an account? <a
                href="<%= request.getContextPath() %>/auth/login"
                class="font-semibold text-sky-700 underline-offset-2 hover:underline">Sign in</a></p></section>
</main>
</body>
</html>
