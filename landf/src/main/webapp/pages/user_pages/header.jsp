                ...
                <a href="${pageContext.request.contextPath}/profile" class="h-9 w-9 rounded-full overflow-hidden border border-slate-200 block">
                    <c:choose>
                        <c:when test="${not empty sessionScope.profilePhoto}">
                            <img alt="User profile" class="h-full w-full object-cover" src="${sessionScope.profilePhoto}"/>
                        </c:when>
                        <c:otherwise>
                            <img alt="User profile" class="h-full w-full object-cover" src="https://img.freepik.com/free-photo/closeup-shot-beautiful-butterfly-with-interesting-textures-orange-petaled-flower_181624-7640.jpg?semt=ais_hybrid&w=740&q=80"/>
                        </c:otherwise>
                    </c:choose>
                </a>
            </c:when>
