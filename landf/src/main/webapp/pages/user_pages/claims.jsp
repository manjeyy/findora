...
<main class="ml-64 min-h-screen flex flex-col">
    <div class="mx-auto max-w-7xl px-6 py-10"><% request.setAttribute("pageTitle", "Claim Verification Ledger");
        ...
        <section class="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            <article class="rounded-2xl border border-slate-200 bg-white p-5"><p
                    class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Claims</p>
                <p class="mt-3 text-4xl font-bold text-slate-900"><%= claims.size() %>
                </p></article>
            <article class="rounded-2xl border border-slate-200 bg-white p-5"><p
                    class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Pending Review</p>
                <p class="mt-3 text-4xl font-bold text-slate-900"><%= pendingClaims %>
                </p></article>
            <article class="rounded-2xl border border-slate-200 bg-white p-5"><p
                    class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Resolved</p>
                <p class="mt-3 text-4xl font-bold text-slate-900"><%= claims.size() - pendingClaims %>
                </p></article>
            <article class="rounded-2xl border border-slate-200 bg-white p-5"><p
                    class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Workflow</p>
                <p class="mt-3 text-4xl font-bold text-slate-900">Approve</p></article>
        </section>
        <section class="mt-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div class="flex flex-wrap items-end justify-between gap-4">
                <div><h2 class="text-2xl font-bold tracking-tight">Claims under review</h2>
                    <p class="mt-1 text-sm text-slate-600">Open a proof block, then approve or reject the claim from the
                        same row.</p></div>
            </div>
            <div class="mt-6 overflow-x-auto rounded-2xl border border-slate-200">
                <table class="min-w-full divide-y divide-slate-200 text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100 bg-white"><% for (AdminClaimView claim : claims) { %>
                    <tr class="align-top hover:bg-slate-50">
                        <td class="px-4 py-4"><p
                                class="font-semibold text-slate-900"><%= escapeHtml(claim.getItemTitle()) %>
                        </p>
                            <p class="mt-1 text-xs text-slate-500">Claim ID: <%= claim.getClaimId() %>
                            </p>
                            <p class="text-xs text-slate-500">Item ID: <%= claim.getItemId() %>
                                | <%= escapeHtml(claim.getItemType()) %>
                            </p>
                            <p class="text-xs text-slate-500">Item Status: <%= escapeHtml(claim.getItemStatus()) %>
                            </p>
                            <p class="text-xs text-slate-500">Claimant: <%= escapeHtml(claim.getClaimantUsername()) %>
                            </p>
                            <p class="text-xs text-slate-500">Submitted: <%= escapeHtml(claim.getCreatedAt()) %>
                            </p></td>
                        <td class="px-4 py-4">
                            <details class="rounded-2xl border border-slate-200 bg-slate-50 p-3">
                                <summary
                                        class="cursor-pointer text-xs font-semibold uppercase tracking-[0.15em] text-slate-700">
                                    Ownership proof
                                </summary>
                                <p class="mt-3 whitespace-pre-line text-xs leading-6 text-slate-600"><%= escapeHtml(claim.getProof()) %>
                                </p></details>
                            <% if (claim.getIdentifiers() != null && !claim.getIdentifiers().isBlank()) { %> <p
                                class="mt-3 text-xs text-slate-600"><span
                                class="font-semibold">Identifiers:</span> <%= escapeHtml(claim.getIdentifiers()) %>
                        </p> <% } %></td>
                        <td class="px-4 py-4"><span
                                class="rounded-full border border-slate-300 bg-slate-50 px-3 py-1 text-xs font-semibold text-slate-700"><%= escapeHtml(claim.getStatus()) %></span> <% if (claim.getReviewedBy() != null && !claim.getReviewedBy().isBlank()) { %>
                            <p class="mt-2 text-xs text-slate-500">Reviewed by <%= escapeHtml(claim.getReviewedBy()) %>
                            </p> <% } %> <% if (claim.getReviewNotes() != null && !claim.getReviewNotes().isBlank()) { %>
                            <p class="mt-1 text-xs text-slate-600"><%= escapeHtml(claim.getReviewNotes()) %>
                            </p> <% } %></td>
                        <td class="px-4 py-4"><% if ("pending".equalsIgnoreCase(claim.getStatus())) { %>
                            <form method="post" action="<%= contextPath %>/admin/claims/review" class="space-y-2"><input
                                    type="hidden" name="claimId" value="<%= claim.getClaimId() %>"> <input type="text"
                                                                                                           name="notes"
                                                                                                           maxlength="255"
                    ...
                    <% } %> <% if (claims.isEmpty()) { %>
                    <tr>
                        <td colspan="4" class="px-4 py-10 text-center text-sm text-slate-500">No claim records
                            available.
                        </td>
                    </tr>
                    <% } %></tbody>
